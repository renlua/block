...
function newRemote(type, name, args, remote, function_info, blocked, src, returnValue)
	...
	local log = {
		...
		Blocked = blocked,
		Source = src,
		GenScript = "汉化作者方法：拼手速，一边点重复的事件，点成功了就点排除或阻止 没有效果清除日志再来一次\n——生成，请等待……(单击重新加载)\n——(如果此消息仍然存在，则远程参数可能非常长)",
...
newButton("执行事件代码", function()
	return "点击执行此事件代码"
end, function()
	local orText = "点击来执行这串代码"
	TextLabel.Text = "执行..."
...
newButton("函数信息", function()
	...
	if selected then
		...
		TextLabel.Text = "完成了!由Spy序列化器生成的函数信息."
	end
end)

--- Clears the Remote logs
newButton("清除日志", function()
	return "点击清除日志"
end, function()
	TextLabel.Text = "清除..."
	logs = {}
...
newButton("排除远程出现1", function()
	return "单击以排除所有远程。排除远程使Spy忽略它，但它将继续可用。"
end, function()
	if selected then
		blacklist[selected.Remote.remote] = true
		TextLabel.Text = "被排除在外"
	end
end)

--- Excludes all Remotes that share the same name as the selected.Log remote from the RemoteSpy
newButton("排除远程出现2", function()
	return "单击以排除具有此名称的所有远程。排除远程使Spy忽略它，但它将继续可用。"
end, function()
	if selected then
...
--- clears blacklist
newButton("清除黑名单", function()
	return "点击清除黑名单。排除远程使Spy忽略它，但它将继续可用."
end, function()
	blacklist = {}
	TextLabel.Text = "清除黑名单列表"
end)

--- Prevents the selected.Log Remote from firing the server (still logged)
newButton("阻止远程发生1", function()
	return "点击阻止此远程发生。阻止一个远程不会从Spy日志中删除它，但它不会继续触发服务器."
end, function()
	if selected then
		if selected.Remote.remote then
			...
			TextLabel.Text = "错误!实例可能不再存在，请尝试使用阻止远程发生"
		end
	end
end)

--- Prevents all remotes from firing that share the same name as the selected.Log remote from the RemoteSpy (still logged)
newButton("阻止远程发生2", function()
	return "点击可阻止具有此名称的远程启动。阻塞一个远程不会从Spy日志中删除它，但它不会继续触发服务器。"
end, function()
	if selected then
...
--- clears blacklist
newButton("清除阻止列表", function()
	return "点击停止阻止远程。阻止一个远程不会从Spy日志中删除它，但它不会继续触发服务器"
end, function()
	blocklist = {}
	TextLabel.Text = "清除阻止名单列表"
end)
...
newButton("反编译", function()
	...
	if selected then
		if selected.Source then
			...
			TextLabel.Text = "来源未找到"
		end
	end
end)

newButton("切换信息", function()
	return string.format(
		"[%s] 切换功能信息(因为它会导致某些游戏延迟)",
...
newButton("自动阻止", function()
	return string.format(
		"[%s] [BETA] 从日志中智能检测和排除垃圾远程调用",
		autoblock and "ENABLED" or "DISABLED"
	)
end, function()
	autoblock = not autoblock
	TextLabel.Text = string.format(
		"[%s] [BETA]从日志中智能地检测和排除垃圾远程呼叫",
		autoblock and "ENABLED" or "DISABLED"
	)
	history = {}
	excluding = {}
...
newButton("调通脚本", function()
	return string.format(
		"[%s] [UNSAFE] 使用'获取调用脚本'获取反编译和获取脚本的调用脚本。更可靠，但也使Spy容易被检测和/或不稳定.",
		useGetCallingScript and "ENABLED" or "DISABLED"
	)
end, function()
	useGetCallingScript = not useGetCallingScript
	TextLabel.Text = string.format(
		"[%s] 使用'获取调用脚本'获取反编译和获取脚本的调用脚本。更可靠，但也使Spy容易被检测和/或不稳定。",
		useGetCallingScript and "ENABLED" or "DISABLED"
	)
end)

newButton("关键字符串", function()
	return string.format(
		"[%s] [BETA] ，使用一个实验性的新函数来复制Roblox在使用非原始类型作为表中的键时的行为。仍在开发中，可能无法正确反映字符串(空)用户数据.",
		keyToString and "ENABLED" or "DISABLED"
	)
end, function()
	keyToString = not keyToString
	TextLabel.Text = string.format(
		"[%s][BETA]使用一个实验性的新函数来复制Roblox的行为，当一个非原始类型被用作表中的键时。仍在开发中，可能无法正确反映字符串(空)用户数据。",
		keyToString and "ENABLED" or "DISABLED"
	)
end)

newButton("切换返回值", function()
	return string.format(
		"[%s] [EXPERIMENTAL] 允许记录'获取返回值'的返回值\n\n使用此方法的风险由您自己承担，因为它可能被检测到.",
		recordReturnValues and "ENABLED" or "DISABLED"
	)
end, function()
	recordReturnValues = not recordReturnValues
	TextLabel.Text = string.format(
		"[%s][EXPERIMENTAL]启用记录'获取返回值'的返回值\n\n使用此方法风险自负，因为它可能被检测到.",
		recordReturnValues and "ENABLED" or "DISABLED"
	)
end)

newButton("获取返回值", function()
	return "[Experimental] 如果启用了“返回值”，这将显示远程函数的记录返回值(如果可用)。."
end, function()
	if selected then
